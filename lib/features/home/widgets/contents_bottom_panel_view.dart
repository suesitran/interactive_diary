...
class ContentsBottomPanelView extends StatefulWidget {
  ...
  const ContentsBottomPanelView({required this.onDragClosed, Key? key})
      : super(key: key);
                          ...
                          return ListView.separated(
                              ...
                              itemBuilder: (_, int idx) => Column(
                                    children: <Widget>[
                                      ...
                                      if (idx ==
                                          state.getContents.length -
                                              1) ...<Widget>[const Gap.v16()]
                                    ],
                                  ),
...
class ContentsBottomPanelViewV2State extends State<ContentsBottomPanelViewV2> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final RenderBox? box = locationAddressPanelKey.currentContext
    ...
    return NotificationListener<DraggableScrollableNotification>(
      onNotification: ((DraggableScrollableNotification notification) {
        ...
        if (notification is ScrollEndNotification) {
          print(
              'notification : ${notification.extent} | ${notification.depth}');
                ...
                children: <Widget>[
                  ...
                  Container(
                      margin: EdgeInsets.only(
                        left: screenHorizontalSpacing,
                        right: screenHorizontalSpacing,
                        // bottom: MediaQuery.of(context).viewPadding.bottom
                      ),
                      child: LocationView(
                        currentLocation: widget.address,
                      ) // TODO check why there are 2 LocationView widget used?
                      ),
                  const Gap.v20(),
                  const Gap.v04(),
...
class LocationAddressPanel extends StatelessWidget {
  ...
  const LocationAddressPanel(
      {required this.address, required this.screenHorizontalSpacing, Key? key})
      : super(key: key);
    ...
    return Column(
      ...
      children: <Widget>[
                ...
                children: <Widget>[
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: NartusColor.grey,
                        borderRadius:
                            BorderRadius.circular(Dimension.spacing2)),
                  )
                ],
              ),
            )
        ...
        Container(
          ...
          child: LocationView(
              currentLocation:
                  address), // TODO check why there are 3 location view widget used?
        ),
        const Gap.v20(),
        const Gap.v04(),
      ],
