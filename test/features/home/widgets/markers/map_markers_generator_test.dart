...
void main(){
  ...
  test('verify menu position on animation', () async {
    bool stream = false;
    final MapMarkerGenerator generator = MapMarkerGenerator(
      currentLocation: const LatLng(0.0, 0.0),
      onCurrentLocationMarkerTapped: () {  },
    ...
    generator.markerData.listen((markers) {
      ...
      // camera marker
      final Marker camera = markers.firstWhere((element) => element.markerId.value == menuCameraMarkerLocationId);
      expect(camera.anchor, const Offset(15, 20));

      // pen marker
      final Marker pen  = markers.firstWhere((element) => element.markerId.value ==  menuPencilMarkerLocationId);
      expect(pen.anchor, const Offset(16, 30));

      // mic marker
      final Marker mic = markers.firstWhere((element) => element.markerId.value == menuVoiceMarkerLocationId);
      expect(mic.anchor, const Offset(17, 40));

      // smiley marker
      final  Marker smiley = markers.firstWhere((element) => element.markerId.value == menuEmojiMarkerLocationId);
    ...
    generator.onAnimation(animValue: 0.8,
