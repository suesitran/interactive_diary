...
void main() {
  group('Test vertical gap', () {
    testWidgets('When create gap with v20, Then render SizedBox with height 20',
      ...
      final GlobalKey gapKey = GlobalKey();
      final gapV20 = Gap.v20(key: gapKey,);

      /// We need to wrap gap with some widget to prevent SizedBox take all available space
      await tester.pumpWidget(MaterialApp(home: Column(children: [gapV20],),));
      await tester.pumpAndSettle();

      final Size size = gapKey.currentContext!.size!;
    ...
    testWidgets('When create gap with v16, Then render SizedBox with height 16',
      (WidgetTester tester) async {
        final GlobalKey gapKey = GlobalKey();
        final gap = Gap.v16(key: gapKey,);

        /// We need to wrap gap with some widget to prevent SizedBox take all available space
        await tester.pumpWidget(MaterialApp(home: Column(children: [gap],),));
        await tester.pumpAndSettle();
